...
class MLPClassifier:
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        hidden_layer_output = np.zeros([xShape[0], 3])
        output_layer_output = np.zeros([xShape[0], 3])
        
        for d in range(xShape[0]):
            hidden_layer_output[d] = sigmoid(self.W_bias[0] + np.matmul(x[d], self.W))
        
        for d in range(xShape[0]):
            output_layer_output[d] = softmax(np.matmul(hidden_layer_output[d], self.GAMMA) + self.GAMMA_bias)
    ...
    def train(self, data_instances: np.ndarray, labels: np.ndarray):
        ...
        for iteration_count in range(1, self.epoch_number+1):
            ...
            for data_index in range(len(data_instances)):
                ...
                GAMMA_bias_update = output_layer_output - label
                GAMMA_update = np.matmul(np.transpose(hidden_layer_output), GAMMA_bias_update)
                
                
                # updating first layer weights
                W_bias_update = np.matmul(GAMMA_bias_update, np.transpose(self.GAMMA)) * (hidden_layer_output * (1-hidden_layer_output))
                W_update = np.matmul(np.transpose(x), W_bias_update)
