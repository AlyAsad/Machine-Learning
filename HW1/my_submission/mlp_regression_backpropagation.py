...
class MLPRegressor:
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        output_layer_output = np.zeros([xShape[0], 1])
        
        for d in range(xShape[0]):
            hidden_layer_output[d] = sigmoid(self.W_bias[0] + np.matmul(x[d], self.W))
        
        for d in range(xShape[0]):
            output_layer_output[d][0] = np.matmul(hidden_layer_output[d], self.GAMMA) + self.GAMMA_bias[0][0]
    ...
    def train(self, data_instances: np.ndarray, labels: np.ndarray):
        ...
        for iteration_count in range(1, self.epoch_number+1):
            ...
            for data_index in range(len(data_instances)):
                ...
                GAMMA_bias_update = (-2) * (label - output_layer_output)
                GAMMA_update = GAMMA_bias_update[0][0] * np.transpose(hidden_layer_output)
                
                # updating first layer weights
                W_bias_update = GAMMA_bias_update[0][0] * (np.transpose(self.GAMMA) * (hidden_layer_output * (1-hidden_layer_output)))
