...
def forward_pass(w1, b1, w2, b2, input_data):
    ...
    h = torch.tanh(torch.matmul(input_data, w1) + b1)
    
    output_layer_output = torch.softmax(torch.matmul(h, w2) + b2, 1)
...
for iteration in range(1, ITERATION+1):
    ...
    train_mean_crossentropy_loss = torch.mean(-torch.sum(train_label * torch.log(train_predictions), dim = 1))
    ...
    with torch.no_grad():
        ...
        train_accuracy = torch.mean((torch.argmax(train_label, dim = 1) == torch.argmax(train_predictions, dim = 1)).float())*100
        ...
        validation_mean_crossentropy_loss = torch.mean(-torch.sum(validation_label * torch.log(validation_predictions), dim = 1))
        ...
        validation_accuracy = torch.mean((torch.argmax(validation_label, dim = 1) == torch.argmax(validation_predictions, dim = 1)).float())*100
...
with torch.no_grad():
    ...
    test_accuracy = torch.mean((torch.argmax(test_label, dim = 1) == torch.argmax(test_predictions, dim = 1)).float()) *100
